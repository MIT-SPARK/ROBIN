...
const std::map<INPUT_PARAMS, mexTypeCheckFunction> INPUT_PARMS_MAP{
    {INPUT_PARAMS::measurements, &isPointCloudMatrix},
    {INPUT_PARAMS::so3_dist_type, &isRealDoubleScalar},
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  // Prepare parameters
  auto measurements = mexRotAvgMatrixToMatrixVector(prhs[toUType(INPUT_PARAMS::measurements)]);
  auto so3_dist_type_num =
      static_cast<double>(*mxGetPr(prhs[toUType(INPUT_PARAMS::so3_dist_type)]));
  auto noise_bound = static_cast<double>(*mxGetPr(prhs[toUType(INPUT_PARAMS::noise_bound)]));
  robin::So3Distance so3_dist_type = so3_dist_type_num == 0 ? robin::So3Distance::CHORDAL_DISTANCE
                                                            : robin::So3Distance::GEODESIC_DISTANCE;
  mexPrintf("Parameters prepared.\n");
  mexEvalString("drawnow;");
  ...
  // Call robin API
  auto g = robin::MakeRotAvgInvGraph(measurements, so3_dist_type, noise_bound);

  mexPrintf("Graph made.\n");
  mexEvalString("drawnow;");
